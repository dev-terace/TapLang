...
export type MemberActionMode = 'transferOwner' | 'kickMember' | 'all'
...
const loadMembers = async () => {
    ...
    try {
        ...
        const data = await chatRoomStore.getGroupChatMembers(conversationId)
...
const toggleMember = (index: number) => {
    expandedMemberIndex.value =
        expandedMemberIndex.value === index ? null : index
}
</script>
...
<template>
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"
        ...
        <!-- Modal -->
        <div
            class="w-full max-w-sm bg-[#e6e2db] border-4 border-[#2d2b28] shadow-[8px_8px_0px_0px_#121315] overflow-hidden flex flex-col max-h-[80vh]">
            <!-- Header -->
            <div
                class="bg-[#2d2b28] text-[#fbf9f5] px-4 py-1.5 flex justify-between items-center text-xs font-bold shrink-0">
                <span> {{ headerTitle }} </span>

                <button type="button" class="hover:text-red-400 font-pixel text-lg leading-none transition-colors"
                    @click="emit('close')">
                    ×
                </button>
            </div>
                ...
                <div class="flex-1 overflow-y-auto space-y-2.5 pr-1">
                    ...
                    <template v-else>
                        <div v-for="(member, index) in members" :key="member.id || index"
                            ...
                            :class="expandedMemberIndex === index
                                    ? 'bg-[#3d3a36] text-[#fbf9f5]'
                                    : 'bg-[#f4f1eb]'
                                ">
                            <!-- Member Profile -->
                            <div @click="toggleMember(index)"
                                ...
                                <!-- Flag -->
                                <div
                                    class="relative w-7 h-7 flex items-center justify-center border border-[#2d2b28] shrink-0 bg-[#2d2b28]">
                                    <img :src="`https://flagcdn.com/w40/${member.flag || 'kr'}.png`" alt=""
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-1.5">
                                        <span class="w-1.5 h-1.5 inline-block rounded-full bg-emerald-500" />
                                        <span class="text-xs font-bold truncate tracking-tight">
                                            {{ member.name }}
                                        </span>
                                    </div>

                                    <div class="text-[9px] truncate mt-0.5" :class="expandedMemberIndex === index
                                            ? 'text-[#c5bfb6]'
                                            : 'text-neutral-500'
                                        ">
                                ...
                                <div class="text-[8px] opacity-60 px-1">
                                    ...
                                    <span v-else
                                        class="group-hover:translate-y-0.5 inline-block transition-transform">▼</span>
                                </div>
                            </div>

                            <!-- Member Actions -->
                            <Transition name="accordion">
                                <div v-show="expandedMemberIndex === index"
                                    class="flex bg-[#2d2b28] text-[#fbf9f5] text-[10px] font-bold border-t-2 border-[#2d2b28] divide-x divide-[#4a4641]">
                                    <!-- 방장 위임 버튼 (mode가 'all' 이거나 'transferOwner'일 때) -->
                                    <button v-if="mode === 'all' || mode === 'transferOwner'" type="button" @click.stop="
                                    ...
                                    <button v-if="mode === 'all' || mode === 'kickMember'" type="button"
