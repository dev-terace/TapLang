...
// 폼 데이터 (언어를 배열 형태로 변경하여 여러 개 추가 가능하도록 설정)
      ...
      >
        ...
        <div class="p-5 overflow-y-auto custom-scrollbar space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div class="flex gap-2 mb-2">
                  <input 
                    type="text" 
                    v-model="newSpokenLang" 
                    @keyup.enter="addSpokenLang"
                ...
                <div class="flex flex-wrap gap-2 min-h-[24px]">
                  <span 
                    v-for="(lang, index) in bioFormData.spokenLangs" 
                    :key="index"
                  ...
                  >
                    {{ lang }}
                    <button @click="removeSpokenLang(index)" class="text-rose-600 hover:text-rose-800 ml-1 leading-none">×</button>
                  </span>
                  <span v-if="bioFormData.spokenLangs.length === 0" class="text-[10px] text-neutral-400">등록된 언어가 없습니다.</span>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="flex gap-2 mb-2">
                  <input 
                    type="text" 
                    v-model="newLearningLang" 
                    @keyup.enter="addLearningLang"
                    list="language-suggestions"
                    placeholder="언어 입력 후 Enter"
                    class="flex-1 bg-white border-2 border-[#2d2b28] p-2 text-xs outline-none shadow-inner"
                  />
                  <button 
                    @click="addLearningLang"
                    class="bg-[#2d2b28] text-white border-2 border-[#2d2b28] px-3 text-xs font-bold hover:bg-neutral-800 transition-colors shadow-[2px_2px_0px_0px_#a39b90]"
                  >
                    추가
                  </button>
                ...
                <div class="flex flex-wrap gap-2 min-h-[24px]">
                  <span 
                    v-for="(lang, index) in bioFormData.learningLangs" 
                    :key="index"
                    class="flex items-center gap-1 bg-[#d1cbc1] border border-[#2d2b28] px-2 py-1 text-[10px] font-bold shadow-[1px_1px_0px_0px_#2d2b28]"
                  >
                    {{ lang }}
                    <button @click="removeLearningLang(index)" class="text-rose-600 hover:text-rose-800 ml-1 leading-none">×</button>
                  </span>
                  <span v-if="bioFormData.learningLangs.length === 0" class="text-[10px] text-neutral-400">등록된 언어가 없습니다.</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              v-model="bioFormData.bio"
              rows="4"
              placeholder="자신을 자유롭게 소개해주세요."
              class="w-full bg-white border-2 border-[#2d2b28] p-3 text-xs outline-none shadow-inner resize-none custom-scrollbar leading-relaxed"
            ></textarea>
          </div>
          ...
          <div>
            ...
            <div class="space-y-2">
              <div 
                v-for="(link, index) in bioFormData.snsLinks" 
                :key="index"
              ...
              <div v-if="bioFormData.snsLinks.length === 0" class="text-center py-3 text-xs text-neutral-400 border border-dashed border-[#c5bfb6]">
                등록된 외부 연결망이 없습니다.
              </div>
            </div>
          </div>
