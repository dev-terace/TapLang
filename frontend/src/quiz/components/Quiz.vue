  ...
  <div ref="scrollContainer" v-if="uiStore.currentTab === 'quiz'" class="flex-1 h-screen overflow-y-auto p-6 md:p-10 bg-[#f7f5ed] text-slate-800">
    ...
    <div v-if="subView === 'menu'" class="max-w-4xl mx-auto py-4">
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-2">
          <button @click="subView = 'shared'" class="bg-white hover:bg-slate-50 border-2 border-slate-800 rounded-xl px-3.5 py-2 text-xs font-bold shadow-[2px_2px_0px_0px_#1e293b]">
            🌐 공유 게시판
          </button>
          <button @click="openCreateMode" class="bg-amber-400 hover:bg-amber-300 border-2 border-slate-800 rounded-xl px-3.5 py-2 text-xs font-bold shadow-[2px_2px_0px_0px_#1e293b]">
            + 새 컬렉션
          </button>
        </div>
      </div>
            ...
            <div class="flex justify-between items-center mb-2">
              ...
              <div class="flex space-x-1">
                <template v-if="!getAuthorName(col.author).includes('(가져옴)')">
                  ...
                  <button @click="openEditCollection(col)" class="bg-amber-100 text-amber-900 border border-amber-800 text-[10px] font-bold px-2 py-0.5 rounded">
                    ✏️ 수정
                  </button>
                </template>
    ...
    <div v-else-if="subView === 'practice'" class="max-w-2xl mx-auto">
      <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-slate-300">
        ...
        <button @click="subView = 'menu'" class="bg-white border-2 border-slate-800 rounded-lg px-3.5 py-1.5 text-xs font-bold">← 목록으로</button>
      </div>
      ...
      <div class="bg-white border-2 border-slate-800 rounded-2xl p-6 shadow-[4px_4px_0px_0px_#1e293b]">
        <div class="mb-4 flex items-center justify-between bg-amber-50 p-3 border-2 border-slate-800 rounded-xl">
          ...
          <select v-model="selectedTtsLang" class="bg-white border-2 border-slate-800 rounded-lg text-xs font-bold px-2 py-1 outline-none">
        ...
        <div v-if="quizStore.activeCollection?.sentences?.length">
          <div class="mb-6 bg-slate-50 p-3 border-2 border-slate-800 rounded-xl">
            <select :value="quizStore.selectedSentenceIndex" @change="(e: any) => handleSentenceChange(Number(e.target.value))"
          ...
          <div @click="speak(quizStore.currentSentence.voiceText)"
            ...
            <div class="flex items-end space-x-1.5 h-10">
              <div v-for="(h, idx) in barHeights" :key="idx" class="w-2.5 bg-slate-800 rounded-t-sm transition-all duration-75" :style="{ height: `${h}px` }"></div>
            </div>
          ...
          <div class="flex space-x-2">
            <button @click="checkAnswer(quizStore.currentSentence.voiceText)" class="flex-1 bg-slate-900 text-white font-bold text-xs py-3 rounded-xl border-2 border-slate-800">Check</button>
            <button @click="showAnswer(quizStore.currentSentence.voiceText)" class="bg-white text-slate-800 font-bold text-xs px-5 py-3 rounded-xl border-2 border-slate-800">Show Answer</button>
          </div>
          ...
          <div v-if="resultState.status !== 'none'" class="mt-4 p-3 rounded-xl text-xs font-bold border-2 border-slate-800"
            ...
            <div v-if="resultState.diffs" class="text-[11px] font-mono mt-1 text-rose-700">힌트: {{ resultState.diffs }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="subView === 'shared'" class="max-w-4xl mx-auto">
      <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-slate-300">
        ...
        <div class="flex items-center space-x-2">
          ...
          <button @click="subView = 'menu'" class="bg-white border-2 border-slate-800 rounded-lg px-3.5 py-1.5 text-xs font-bold">← 메인으로</button>
        </div>
      </div>
    ...
    <div v-else-if="subView === 'generator'" class="max-w-3xl mx-auto">
      <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-slate-300">
        ...
        <button @click="resetForm(); subView = 'menu'" class="bg-white border-2 border-slate-800 rounded-lg px-3.5 py-1.5 text-xs font-bold">← 메인으로</button>
      </div>
      ...
      <div class="bg-white border-2 border-slate-800 rounded-2xl p-6 shadow-[4px_4px_0px_0px_#1e293b] space-y-4">
        ...
        <div class="border-t-2 border-slate-100 pt-3">
          ...
          <button @click="addSentenceToForm" class="w-full bg-slate-100 border-2 border-slate-800 rounded-xl py-2 text-xs font-bold">+ 문장 추가</button>
        </div>
